...
public class CarteleraDAOHibernateJPA extends GenericDAOHibernateJPA<Cartelera> implements CarteleraDAO {
	...
	public List<Publicacion> obtenerPublicaciones(long id) {
		Query q = this.getEntityManager().createQuery("Select new Publicacion(p.id, p.titulo, p.descripcion, p.fechaCreacion, p.creador) from Cartelera c JOIN c.publicaciones p Where c.id=:id");		
		q.setParameter("id", id);
